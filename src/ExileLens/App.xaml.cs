...
public partial class App : Application
{
    ...
    protected override void OnStartup(StartupEventArgs e)
    {
        ...
        if (!smoke)
        {
            instance = new Mutex(true, @"Local\ExileLens-Overlay", out bool created);
            if (!created)
            {
                MessageBox.Show("Exile Lens is already running. Use its notification-tray icon to reopen it, or exit the old instance before updating.", "Exile Lens");
