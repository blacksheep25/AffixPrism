...
public partial class MainWindow
{
    ...
    private readonly Button updateInstall=new() { Content="Update and restart", Visibility=Visibility.Collapsed };
    private void InitializeUpdates()
    {
        updateHttp.DefaultRequestHeaders.UserAgent.ParseAdd("ExileLens-Updater/1.0");
        ...
        Option("Check for updates at startup",settings.CheckUpdates,v=>settings.CheckUpdates=v);
        ...
        if(!smoke) Loaded+=async(_,_)=> { if(settings.CheckUpdates)await CheckAppUpdates(); };
    }
    private async Task CheckAppUpdates()
    {
        ...
        try
        {
            ...
            using var response=await updateHttp.GetAsync("https://api.github.com/repos/blacksheep25/ExileLens/releases?per_page=100",lifetime.Token); response.EnsureSuccessStatusCode();
            ...
            if(availableUpdate?.Version!=release?.Version) { updateStage=null; updateInstall.Visibility=Visibility.Collapsed; }
            ...
            if(release!=null)Notice.Text=$"ExileLens {release.Version} available · open Settings → Application updates.";
        }
    ...
    private async Task DownloadAppUpdate()
    {
        ...
        try
        {
            string directory=Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),"ExileLens","updates",Guid.NewGuid().ToString("N")); Directory.CreateDirectory(directory);
            ...
            string stage=Path.Combine(Path.GetDirectoryName(target)!,".ExileLens-update-"+Guid.NewGuid().ToString("N"));
            await Task.Run(()=>GitHubUpdates.Extract(archive,stage),lifetime.Token); updateStage=stage;
            updateStatus.Text=$"{availableUpdate.Version} downloaded and verified. Restart when you are ready.";
            updateInstall.Visibility=Visibility.Visible; Notice.Text=updateStatus.Text;
        }
    ...
    {
        if(updateStage==null||smoke)return;
        try
        {
            string directory=Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),"ExileLens","updates",Guid.NewGuid().ToString("N")); Directory.CreateDirectory(directory);
            ...
            var start=new ProcessStartInfo(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.System),"WindowsPowerShell","v1.0","powershell.exe")) { UseShellExecute=false,CreateNoWindow=true,WindowStyle=ProcessWindowStyle.Hidden };
        ...
        catch(Exception ex) when(ex is IOException or UnauthorizedAccessException or System.ComponentModel.Win32Exception) { updateStatus.Text="Could not start updater: "+ex.Message; }
    }
}
