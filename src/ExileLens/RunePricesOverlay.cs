...
internal sealed class RunePricesOverlay : Window
{
    ...
    public bool Render(System.Drawing.Rectangle region, IReadOnlyList<(RuneText Text,RuneNameMatch Match)> rows, string stamp)
    {
        ...
        {
            var text=new TextBlock { Text=$"≈ {row.Match.PriceLabel}{(row.Match.Approximate ? " · check name" : "")}\n{row.Match.Row.Name}",Foreground=new SolidColorBrush(Color.FromRgb(233,215,141)),FontSize=11,TextWrapping=TextWrapping.Wrap };
            var border=new Border { Child=text,Background=new SolidColorBrush(Color.FromArgb(245,18,18,14)),BorderBrush=new SolidColorBrush(Color.FromRgb(107,92,48)),BorderThickness=new Thickness(1),Padding=new Thickness(6,3,6,3),Width=width/dpi.DpiScaleX-2 };
