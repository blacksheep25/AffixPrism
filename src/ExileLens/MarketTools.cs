...
public partial class MainWindow
{
    ...
    {
        var candidates = result.Rows.Select(row => (Row:row, Estimate:SimilarItems.Estimate(row.Item,result with { Rows = result.Rows.Where(r => !r.Account.Equals(row.Account,StringComparison.OrdinalIgnoreCase)).ToArray() })))
            .Where(x => x.Estimate.Price > x.Row.Listing.Price.Amount)
            .OrderByDescending(x => x.Estimate.Price-x.Row.Listing.Price.Amount).Take(8).ToArray();
        ResaleCandidates.Text = $"{settings.League} · {result.CapturedAt.ToLocalTime():dd MMM HH:mm} · fetched sample only\n" +
            ...
            string.Join("\n\n", candidates.Select(x => $"{x.Row.Item.Name} · {x.Row.Account}\nAsk {x.Row.PriceLabel} → peer median {x.Estimate.Price:0.##} {result.Currency} · gross gap {x.Estimate.Price-x.Row.Listing.Price.Amount:0.##}\n{x.Estimate.Sellers} other sellers · {x.Estimate.Similarity:P0} similarity")))
    ...
    private async void LoadOpportunities(object sender,RoutedEventArgs e)
    {
        ...
        try
        {
            string category=((ComboBoxItem)OpportunityCategory.SelectedItem).Content.ToString()!;
