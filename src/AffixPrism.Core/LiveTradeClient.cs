...
public sealed class LiveTradeClient(HttpClient http)
{
    ...
    private async Task<JsonElement> SendAsync(HttpMethod method, string path, string? body, string policy, string stage, CancellationToken token, Action<string>? progress)
    {
        ...
        message.Headers.UserAgent.ParseAdd("AffixPrism/0.3");
        ...
        { throw new HttpRequestException("Windows blocked AffixPrism's connection to pathofexile.com. Check the app's network permissions.", ex); }
        using (response)
        {
            ...
            if (response.StatusCode is HttpStatusCode.Forbidden or HttpStatusCode.Unauthorized)
                throw new HttpRequestException("The trade site requires sign-in or browser verification. AffixPrism's direct connection cannot complete that verification yet.");
