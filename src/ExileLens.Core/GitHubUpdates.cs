...
{
    public const string Repository="https://github.com/blacksheep25/ExileLens";
    ...
    public static AppRelease? Select(JsonElement releases,string current,bool betas)
    {
        ...
        foreach(var release in releases.EnumerateArray())
        {
            ...
            string name=$"ExileLens-{tag.TrimStart('v')}-win-x64.zip";
    ...
    public static void Extract(string archive,string destination)
    {
        ...
        if(!zip.Entries.Any(e=>e.FullName=="ExileLens.exe"))throw new IOException("Update has no application executable.");
