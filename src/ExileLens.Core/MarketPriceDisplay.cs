...
public static class MarketPriceDisplay
{
    ...
    public static string Format(EconomyRow row, EconomySnapshot? rates, DateTimeOffset now)
        => FormatQuantity(row, 1, rates, now);
    public static string FormatQuantity(EconomyRow row, decimal quantity, EconomySnapshot? rates, DateTimeOffset now)
    {
        if (row.Currency == "Divine Orb" && row.Value is > 0 and < 1 && rates is { Stale:false } && now >= rates.FetchedAt && now-rates.FetchedAt <= TimeSpan.FromHours(1))
        {
            ...
            if (exalt != null) return Amount(quantity*row.Value/exalt.Value)+" Exalted Orb";
        }
        return Amount(quantity*row.Value)+" "+row.Currency;
    }
}
