    ...
    private void Render()
    {
        ...
        foreach(var original in ItemSockets.From(Item))
        {
            ...
            Border Details() {
                ...
                return new Border { Child=panel,Width=300,Padding=new Thickness(12),Background=new SolidColorBrush(Theme.Colour("Surface")),BorderBrush=Theme.Brush("Border"),BorderThickness=new Thickness(1) };
            }
            var contents=new Grid();
            if(socket.Occupied==false) contents.Children.Add(new Image { Source=new System.Windows.Media.Imaging.BitmapImage(new System.Uri("pack://application:,,,/AffixPrism;component/Assets/empty-socket.png")), Width=38,Height=38,Stretch=Stretch.Uniform });
            ...
            var button = new Button { Width=40,Height=40,Padding=new Thickness(0),BorderThickness=new Thickness(2),BorderBrush=new SolidColorBrush(Theme.Colour("Border")),Background=new SolidColorBrush(Theme.Colour("Background")),Margin=new Thickness(4,6,4,6),Content=contents,Cursor=Cursors.Hand };
