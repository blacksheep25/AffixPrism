...
{
    public static string FilePath => Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "AffixPrism", "settings.json");
    public static Settings Load(out string? error)
    {
        ...
        try
        {
            ...
            settings.PriceCheckHotkey = AffixPrism.Core.CheckHotkey.Parse(settings.PriceCheckHotkey)?.Label ?? "Alt+E";
            return settings;
        }
