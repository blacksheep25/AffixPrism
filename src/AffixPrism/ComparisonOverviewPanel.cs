...
public sealed class ComparisonOverviewPanel : Border
{
    ...
    {
        Background=new SolidColorBrush(Theme.Colour("Surface")); Padding=new Thickness(12); Margin=new Thickness(5,0,5,12);
        var root=new StackPanel();
        Child=new Expander { Header="AT A GLANCE", IsExpanded=false, Foreground=Theme.Brush("Gold"), Content=root };
        ...
        void Render()
        {
            ...
            body.Children.Add(new TextBlock { Text=result.Verdict.Replace("Other item",otherLabel), FontSize=16, FontWeight=FontWeights.SemiBold, Foreground=Theme.Brush("Text"), TextWrapping=TextWrapping.Wrap, Margin=new Thickness(0,0,0,8) });
            ...
            table.RowDefinitions.Add(new RowDefinition()); Cell("Key stat",0,0,Theme.Brush("Text")); Cell("Your item",0,1,Theme.Brush("Text")); Cell(otherLabel,0,2,Theme.Brush("Text")); Cell("Difference",0,3,Theme.Brush("Text"));
            int row=1;
            ...
            foreach(var text in new[]{"Higher on other item: "+(result.Gains.Length>0 ? result.Gains : "None"),"Lower on other item: "+(result.Losses.Length>0 ? result.Losses : "None"),"Green/red means higher/lower, not necessarily better/worse. Missing stats are unknown. Choose a priority to narrow the comparison."})
                body.Children.Add(new TextBlock { Text=text,Foreground=Theme.Brush("Muted"),TextWrapping=TextWrapping.Wrap,Margin=new Thickness(0,5,0,0),FontSize=11 });
        }
