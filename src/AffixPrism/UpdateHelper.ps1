...
try {
    if($target -eq [IO.Path]::GetPathRoot($target) -or -not(Test-Path -LiteralPath (Join-Path $target 'AffixPrism.exe')) -or -not(Test-Path -LiteralPath (Join-Path $stage 'AffixPrism.exe'))) { throw 'Invalid update paths' }
    if((Split-Path -Parent $stage) -ne (Split-Path -Parent $target) -or -not((Split-Path -Leaf $stage).StartsWith('.AffixPrism-update-'))) { throw 'Unexpected staging location' }
    ...
    if(Get-Process -Id $job.ProcessId -ErrorAction SilentlyContinue) { throw 'AffixPrism did not exit' }
    Move-Item -LiteralPath $target -Destination $backup
    try {
        Move-Item -LiteralPath $stage -Destination $target
        $process=Start-Process -FilePath (Join-Path $target 'AffixPrism.exe') -WorkingDirectory $target -WindowStyle Hidden -PassThru
    ...
    if(-not(Get-Process -Id $job.ProcessId -ErrorAction SilentlyContinue) -and (Test-Path -LiteralPath (Join-Path $target 'AffixPrism.exe'))) { Start-Process -FilePath (Join-Path $target 'AffixPrism.exe') -WorkingDirectory $target -WindowStyle Hidden }
}
