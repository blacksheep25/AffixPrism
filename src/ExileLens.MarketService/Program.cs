...
var directory = builder.Configuration["data-dir"] ?? Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "ExileLens", "market-service");
...
app.Use(async (context, next) =>
{
    ...
    if (context.Request.Method != "GET" && context.Request.Headers["X-ExileLens-Client"] != "desktop") { context.Response.StatusCode = 403; return; }
    await next();
});
app.MapGet("/health", () => Results.Ok(new { service = "ExileLens Market Service", version = 1, source = "imported listings", liveCollection = false }));
