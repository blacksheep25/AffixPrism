...
Settings → Application updates checks this project's GitHub releases at startup by default. Manual checks are available. Automatic downloads are optional and off by default; beta releases are included by default while AffixPrism is in beta. Installing always requires **Update and restart**.
...
## Pending updates and visibility

Verified downloads are persisted per installation. A normal restart applies the pending update, while an installation failure remains visible rather than triggering repeated retries. The main window shows a persistent update banner with a download/install action. Automatic checks run at startup and every 30 minutes when enabled, with a tray notification for a newly discovered version. The helper runs outside the installation directory so it can rename it.

See [the rename transition](REBRANDING.md) before publishing renamed packages.
