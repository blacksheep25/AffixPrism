...
static class UpdateChecks
{
    public static void Run(Action<bool,string> check)
    {
        ...
        object Release(string version,bool beta,bool draft=false,string host="https://github.com/blacksheep25/AffixPrism")=>new { tag_name=version,prerelease=beta,draft,body="Changes",assets=new[]{ new { name=$"AffixPrism-{version}-win-x64.zip",browser_download_url=$"{host}/releases/download/{version}/AffixPrism-{version}-win-x64.zip" },new { name=$"AffixPrism-{version}-win-x64.zip.sha256",browser_download_url=$"{host}/releases/download/{version}/AffixPrism-{version}-win-x64.zip.sha256" } } };
        ...
        string root=Path.Combine(Path.GetTempPath(),"AffixPrism-update-test-"+Guid.NewGuid().ToString("N")); Directory.CreateDirectory(root);
        try
        {
            string archive=Path.Combine(root,"update.zip");
            using(var zip=ZipFile.Open(archive,ZipArchiveMode.Create)) { using var writer=new StreamWriter(zip.CreateEntry("AffixPrism.exe").Open()); writer.Write("fixture"); }
            ...
            GitHubUpdates.Extract(archive,Path.Combine(root,"stage")); check(File.ReadAllText(Path.Combine(root,"stage","AffixPrism.exe"))=="fixture","Updater stages valid archive");
            string bad=Path.Combine(root,"bad.zip"); using(var zip=ZipFile.Open(bad,ZipArchiveMode.Create)) { zip.CreateEntry("../escape.txt"); zip.CreateEntry("AffixPrism.exe"); }
